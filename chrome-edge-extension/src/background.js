...
class BackgroundService {
  ...
  async handleMessage(message, sender, sendResponse) {
    ...
    try {
      switch (message.type) {
        case 'OBFUSCATE_TEXT':
          console.log('Background: Processing OBFUSCATE_TEXT request');
          const obfuscatedText = await this.obfuscateText(message.text, message.url);
          console.log('Background: Sending obfuscated response');
      ...
      console.error('Background service error stack:', error.stack);
  ...
  async obfuscateText(text, url) {
    console.log('Background: obfuscateText called, enabled:', this.settings.enabled);
    
    if (!this.settings.enabled) {
      console.log('Background: Extension disabled, returning original text');
      return text;
    }

    // Check if current site should be processed
    if (url && !this.shouldProcessSite(url)) {
      console.log('Background: Site should not be processed:', url);
      return text;
    }
    
    console.log('Background: Processing text, simple mode:', this.settings.simpleMode);
    ...
    if (this.advancedDLP && !this.settings.simpleMode) {
      ...
      console.log('Background: Using advanced DLP');
      const result = this.advancedDLP.processText(text, dlpSettings);
      console.log('Background: Advanced DLP result length:', result.length);
      return result;
    } else {
      // Use basic obfuscation for simple mode
      console.log('Background: Using basic obfuscation patterns');
      ...
      console.log('Background: Basic obfuscation result preview:', result.substring(0, 200) + '...');
      return result;
    }
  }
